...
bool decrypt_ELL_AES_CTR(Telegram *t, std::vector<uchar> &frame,
                         std::vector<uchar>::iterator &pos,
                         std::vector<uchar> &aeskey) {
  if (aeskey.size() == 0)
    return true;

  std::vector<uchar> encrypted_bytes;
  std::vector<uchar> decrypted_bytes;
  ...
  for (size_t offset = 0; offset < encrypted_bytes.size(); offset += 16) {
    ...
    std::vector<uchar> tmpv(tmp, tmp + block_size);
...
bool decrypt_TPL_AES_CBC_IV(Telegram *t, std::vector<uchar> &frame,
                            std::vector<uchar>::iterator &pos,
                            std::vector<uchar> &aeskey,
  ...
  *num_encrypted_bytes = num_bytes_to_decrypt;
  ...
  if (buffer.size() < num_bytes_to_decrypt) {
    warning("(TPL) warning: aes-cbc-iv decryption received less bytes than "
            "expected for decryption! "
            "Got %zu bytes but expected at least %zu bytes since num encr "
            "blocks was %d.\n",
  ...
  *num_not_encrypted_at_end = buffer.size() - num_bytes_to_decrypt;
  ...
  debug("(TPL) num encrypted blocks %zu (%d bytes and remaining unencrypted "
        "%zu bytes)\n",
        t->tpl_num_encr_blocks, num_bytes_to_decrypt,
  ...
  if (num_bytes_to_decrypt < buffer.size()) {
    frame.insert(frame.end(), buffer.begin() + num_bytes_to_decrypt,
                 buffer.end());
...
bool decrypt_TPL_AES_CBC_NO_IV(Telegram *t, std::vector<uchar> &frame,
                               std::vector<uchar>::iterator &pos,
                               std::vector<uchar> &aeskey,
                               int *num_encrypted_bytes,
                               int *num_not_encrypted_at_end) {
  ...
  *num_encrypted_bytes = num_bytes_to_decrypt;
  if (buffer.size() < num_bytes_to_decrypt) {
    warning("(TPL) warning: aes-cbc-no-iv decryption received less bytes than "
            "expected for decryption! "
            "Got %zu bytes but expected at least %zu bytes since num encr "
            "blocks was %d.\n",
            buffer.size(), num_bytes_to_decrypt, t->tpl_num_encr_blocks);
  ...
  *num_not_encrypted_at_end = buffer.size() - num_bytes_to_decrypt;
  ...
  debug("(TPL) num encrypted blocks %d (%d bytes and remaining unencrypted %d "
        "bytes)\n",
        t->tpl_num_encr_blocks, num_bytes_to_decrypt,
        buffer.size() - num_bytes_to_decrypt);
